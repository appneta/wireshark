 ...
 *
 * $Id: packet-ani-payload.c fklassen $
 *
...
static int proto_ani_payload = -1;
  ...
  if (show_ani_payload && tree) {
    ...
    if (bytes > 0) {
      ...
      if (bytes >= sizeof(ANI_LEGACY_PAYLOAD_SIGNATURE) &&
        ...
        if (check_col(pinfo->cinfo, COL_INFO)) {
          if (bytes == 18)
            col_append_fstr(pinfo->cinfo, COL_INFO, ", PathTest payload - stream=%u (Final)",
                stream);
          else
            col_append_fstr(pinfo->cinfo, COL_INFO, ", PathTest payload - stream=%u seq=%u",
                stream, seq);
        }
        offset += 7;
