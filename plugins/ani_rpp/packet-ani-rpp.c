...
extern void dissect_ip(tvbuff_t *tvb, packet_info *pinfo,
...
enum ResponderHeaderType {
	HDR_LAST = 1, HDR_SEQUENCE, HDR_ERROR, HDR_REQUEST, HDR_REPLY, //5
	HDR_FLOW_CREATE,
	HDR_FLOW_RESPONSE,
	HDR_FLOW_CLOSE,
	HDR_TEST_WEIGHT,
 ...
 */
static int dissect_rtp_header(tvbuff_t *tvb, packet_info *pinfo, int offset,
		proto_tree *ani_rpp_tree) {
	...
	if (sync_src == NO_FLOW) {
		ti = proto_tree_add_text(ani_rpp_tree, tvb, offset, RTP_HEADER_LENGTH,
				"Responder RTP Header: No flow%s",
				marker_set ? ", Marker" : "");
	} else {
		ti = proto_tree_add_text(ani_rpp_tree, tvb, offset, RTP_HEADER_LENGTH,
				"Responder RTP Header: Flow %d%s", sync_src,
				marker_set ? ", Marker" : "");
	}
 ...
 * the header; otherwise add items to the dissector tree.
 */
static int dissect_responder_header(tvbuff_t *tvb, packet_info *pinfo,
		int offset, proto_tree *ani_rpp_tree) {
	int currentHeader, nextHeader;
	guint8 headerLength = 0, mainHeader = 1, mode = 0;
	guint8 cmd_info_flags;
	guint32 id, flow, major, minor, revision, build, first_id, burst_hold_time,
			i, depth;
	...
	while (currentHeader != HDR_LAST && currentHeader < HDR_INVALID) {
		...
		if (ani_rpp_tree && hf_subtrees[currentHeader]) {
			...
			ti = proto_tree_add_text(ani_rpp_tree, tvb, offset, headerLength,
					"<not set>");
			current_tree = proto_item_add_subtree(ti,
					*(hf_subtrees[currentHeader]));
		}

		if (current_tree) {
			proto_tree_add_item(current_tree, hf_ani_rpp_next_header_type, tvb,
					offset, 1, FALSE);
			proto_tree_add_item(current_tree, hf_ani_rpp_header_length, tvb,
					offset + 1, 1, FALSE);
		}
		offset += 2;
		...
		case HDR_SEQUENCE:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_pkt_id, tvb,
						offset, 4, FALSE);

				/* set some text in the info column */
				col_add_fstr(pinfo->cinfo, COL_INFO,
						"Responder Packet: ID %d(0x%x)", id, id);
			}
		...
		case HDR_ERROR:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_error_code, tvb,
						offset, 1, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_next_header_type,
		...
		case HDR_FLOW_CREATE:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_flow_port, tvb,
						offset, 2, FALSE);
		...
		case HDR_FLOW_RESPONSE:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_flow_num, tvb,
						offset, 4, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_flow_port, tvb,
						offset + 4, 2, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_response_status,
						tvb, offset + 6, 2, FALSE);

				/* set some text in the info column */
				col_append_fstr(pinfo->cinfo, COL_INFO,
						" Flow Response (flow ID %d)", flow);
				mainHeader = 0;
			}
			offset += (headerLength - 2);
			break;
		...
		case HDR_TEST_WEIGHT:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_test_weight, tvb,
						offset, 2, FALSE);

				/* set some text in the info column */
				col_append_fstr(pinfo->cinfo, COL_INFO, " (weight %d)",
						weight);
			}
			offset += (headerLength - 2);
			break;
		case HDR_RESPONDERVERSION:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree,
						hf_ani_rpp_responder_version_major, tvb, offset, 4,
						FALSE);
				proto_tree_add_item(current_tree,
						hf_ani_rpp_responder_version_minor, tvb, offset + 4, 4,
						FALSE);
				proto_tree_add_item(current_tree,
						hf_ani_rpp_responder_version_revision, tvb, offset + 8,
						4, FALSE);
				proto_tree_add_item(current_tree,
						hf_ani_rpp_responder_version_build, tvb, offset + 12, 4,
						FALSE);

				/* set some text in the info column */
				col_append_fstr(pinfo->cinfo, COL_INFO,
						", version %d.%d.%d.%d", major, minor, revision,
						build);
			}
			offset += (headerLength - 2);
			break;
		case HDR_COMMAND_INFO:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_first_id, tvb,
						offset, 4, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_burst_size, tvb,
						offset + 4, 2, FALSE);
				if (headerLength > 8) {
					proto_tree_add_item(current_tree, hf_ani_rpp_packet_size,
							tvb, offset + 6, 2, FALSE);
				}
				if (headerLength >= 11) {
					proto_tree_add_item(current_tree, hf_ani_rpp_command_type,
							tvb, offset + 8, 1, FALSE);
					mode = tvb_get_guint8(tvb, offset + 8);
				}
				if (headerLength >= 12) {
					cmd_info_flags = tvb_get_guint8(tvb, offset + 9);
					tf = proto_tree_add_uint(current_tree,
				...
				col_append_fstr(pinfo->cinfo, COL_INFO,
						", First ID %d, Packets %d", first_id, burstsize);
			}
			offset += (headerLength - 2);
			break;
		case HDR_OUTBOUNDARRIVAL:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree,
						hf_ani_rpp_outbound_arrival_bits, tvb, offset, 8,
						FALSE);
		...
		case HDR_RESPONDERHOLDTIME:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_burst_hold_time_us,
						tvb, offset, 4, FALSE);
				//proto_tree_add_item( current_tree, hf_ani_burst_process_time_us, tvb, offset+4, 4, FALSE );

				/* set some text in the info column */
				col_append_fstr(pinfo->cinfo, COL_INFO, ", Hold %u us",
						burst_hold_time);
			}
			offset += (headerLength - 2);
			break;
		case HDR_OUTBOUNDARRIVALTIME:
			...
			if (current_tree) {
				...
				for (; i < depth; i += 4) {
					proto_tree_add_item(current_tree,
							hf_ani_rpp_outbound_arrival_times, tvb, offset + i,
							4, FALSE);
				}
			}
			offset += (headerLength - 2);
		...
		case HDR_LOST_PACKETS:
			if (current_tree) {
				...
				depth = headerLength - 2;
				for (i = 0; i < depth; i += 4) {
					proto_tree_add_item(current_tree, hf_ani_rpp_lost_id, tvb,
							offset + i, 4, FALSE);
				}
		...
		case HDR_SIPPORT:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_sipport, tvb,
						offset, 2, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_ta_id, tvb,
						offset + 2, idLength, FALSE);
			}
			offset += (headerLength - 2);
			break;
		case HDR_PROTOCOL:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_protocol, tvb,
						offset, 4, FALSE);
			}
			offset += (headerLength - 2);
			break;
		case HDR_CONTROLLEDBURST:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_cb_packetcount,
						tvb, offset, 4, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_cb_interpacketgap,
		...
		case HDR_CONTROLLEDBURSTRESPONSE:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree,
						hf_ani_rpp_cb_resp_ratelimitcbrate, tvb, offset, 4,
						FALSE);
				proto_tree_add_item(current_tree,
						hf_ani_rpp_cb_resp_minpacketcount, tvb, offset + 4, 4,
						FALSE);
			}
			offset += (headerLength - 2);
			break;
		case HDR_INBOUNDPACKETATTR:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_inboundpacketcount,
						tvb, offset, 2, FALSE);
				proto_tree_add_item(current_tree, hf_ani_rpp_inboundpacketsize,
						tvb, offset + 2, 2, FALSE);
				burstsize = tvb_get_ntohs(tvb, offset);
				col_append_fstr(pinfo->cinfo, COL_INFO, "/%d (out/in)",
						burstsize);
			}
			offset += (headerLength - 2);
			break;
		case HDR_H323PORT:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_h323port, tvb,
						offset, 2, FALSE);
			}
			offset += (headerLength - 2);
			break;
		case HDR_APPLIANCE_TYPE:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_appliance_type,
						tvb, offset, 1, FALSE);
			}
			offset += (headerLength - 2);
			break;
		case HDR_CUSTOM_APPLIANCE_TYPE:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree,
						hf_ani_rpp_custom_appliance_type, tvb, offset,
						headerLength - 2, FALSE);
			}
			offset += (headerLength - 2);
			break;
		default:
			if (current_tree) {
				...
				proto_tree_add_item(current_tree, hf_ani_rpp_unknown_header,
						tvb, offset, headerLength - 2, FALSE);
 ...
 */
static int dissect_ani_rpp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_) {
	...
	if (tree) {
		...
		call_dissector(payload_handle, tvb_new_subset(tvb, offset, -1, -1),
				pinfo, tree);
	}
...
void proto_reg_handoff_ani_rpp(void) {
	...
	if (!inited) {
		ani_rpp_handle = new_create_dissector_handle(dissect_ani_rpp,
				proto_ani_rpp);
		inited = TRUE;
	} else {
